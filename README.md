...
- 513 个去重后的技术词汇、AI 行业概念和高频英文界面词（数量不设上限）
- 标准释义 + 大白话 + 技术场景 + 科普要点 + 常见搭配 + 易混点
- 每日固定新词、首次认识、自动混合训练、独立到期复习
- 英选中、中选英、拼写/短语排序、听音选词、例句填空
- 例句填空同步显示完整中文译文，训练中的英文题干与英文选项可点击发音
- 训练中可随时查看当前单词详情，并原路返回当前题目
- 首页学习天数可进入学习日历，按日期查看学习记录、答对/答错和错题
- 今日错题完成后可回顾，并自动进入后续学习/复习巩固
- 高频英文界面词会优先出现在前期学习和后续混合训练中
- 首页可查看今日已经学习过的新词
- 首页顶部和 iPhone/PWA 图标使用新版 TechWords Logo
- 错题记录、训练正确率和错题优先复习
...
│
├── words-data.js         # ★ 513个词汇、AI行业概念和高频界面词
...
window.INITIAL_WORDS = [
  ...
  // ... v1.1.2 共513个（去重后并新增AI行业词汇和高频界面词）
];
```
...
- [ ] 旧版500词数据：自动迁移为513个新版词条（另加用户自定义词），学习记录保留
...
v1.1.2 使用网络优先、离线回退策略。在线启动时会请求最新文件，断网时使用缓存。

每次正式发布仍必须同步修改 `sw.js` 的缓存版本号：

```javascript
// sw.js 第1行
const CACHE_NAME = 'techwords-v1.1.2';
```
...
当前513词仍使用 localStorage。词汇和科普字段持续增加后，需要监控实际存储占用；接近浏览器配额前应迁移到 IndexedDB。
...
- 词库扩展到 513 条，新增 session、settings、profile、billing、usage、privacy、connectors 等高频 AI 产品界面词
- 核验并修正 Dockerfile：展示保留官方 `Dockerfile` 写法，朗读按 Docker file 辅助
- 英文朗读增加语音选择、语速控制和缩写/专有名词朗读别名
- 中文拼英文/短语排序题支持再次点击取消、退格、清空和手动提交
- 每日训练混入上一日错词和已学高频界面词
- 错题写入按日期的详细记录，完成页可跳转今日错题
- 首页学习天数可点击进入日历，支持月份切换、日期点击和日期详情
- 数据结构升级为 `schemaVersion: 5`
- Service Worker 缓存升级为 `techwords-v1.1.2`

### v1.1.1（2026-06-22）

- 例句填空题在英文挖空句下显示完整中文翻译
- 英文题干、英文例句、英文选项和拼写字母/词块支持点击发音
- 保留答对、答错后朗读正确英文单词的反馈
- 每个训练题增加“查看单词”，可查看完整资料并返回原题和原进度
- 首页增加“查看今日已学”，展示当天实际接触过的新词
- 今日已学列表支持进入详情并返回列表
- 首页顶部加入透明主体 Logo，不包含图标白底和空白外框
- 更新 iPhone/PWA 的 192px、512px App 图标
- Service Worker 缓存升级为 `techwords-v1.1.1`
